...
class Simulation(object):
    ...
    def run(self):
        ...
        start = time.time()
        self.G = State_Map(self.n_periods, self.n_parts)
        end = time.time()
        ...
        optimalPolicy = mdptoolbox.mdp.PolicyIteration(tms, rms, 0.995)
        start = time.time()
        optimalPolicy.run()
        end = time.time()
    ...
    def setParameters(self, min_price, max_price, n_prices, n_parts, n_periods, verbose = False):
